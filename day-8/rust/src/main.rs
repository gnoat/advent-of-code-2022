...
fn main() {
    ...
    println!("There are {} trees visible from outside the grid.", part_one(&trees, &width, &height));
    println!("The highest scenic score possible is {}.", part_two(&trees, &treepose, &width, &height));
}

fn part_one(trees: &Vec<Vec<usize>>, width: &usize, height: &usize) -> usize {
    iproduct!(1..width-1, 1..height-1)
        .filter(|&(x, y)| ((0..y).map(|i| trees[x][i]).max().unwrap_or(0) < trees[x][y]) ||
...
fn part_two(trees: &Vec<Vec<usize>>, treepose: &Vec<Vec<usize>>, width: &usize, height: &usize) -> usize {
    iproduct!(1..width-1, 1..height-1)
        .map(|(x, y)| [treewalker(&trees, &x, &y), treewalker(&treepose, &y, &x)].into_iter().flatten().product::<usize>())
        .max()
        .unwrap()
}
...
fn read_string_to_vector(data: &str) -> (usize, usize, Vec<Vec<usize>>, Vec<Vec<usize>>) {
    let tree_array: Vec<Vec<usize>> = data.split("\n")
    ...
    let transpose: Vec<Vec<usize>> = (0..tree_array.len())
        .map(|y| (0..tree_array[0].len()).map(|x| tree_array[x][y]).collect::<Vec<usize>>())
        .collect();
